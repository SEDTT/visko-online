...
/**
 * Wrap a ViskoQuery object for the UI.
 * 
 * @author awknaust
 *
 */
class Query {
	 ...
	 */
	public function __construct($userID, $queryText, $formatURI = null, $typeURI = null, $targetFormatURI = null, $targetTypeURI = null, $viewURI = null, $viewerSetURI = null, $artifactURL = null, $parameterBindings = [], $dateSubmitted = null, $id = null) {
		$this->viskoQuery = new ViskoQuery ();
		$this->viskoQuery->init ( $queryText, $formatURI, $typeURI, $targetFormatURI, $targetTypeURI, $viewURI, $viewerSetURI, $artifactURL, $parameterBindings );
	...
	public function submit() {
		...
		try {
			list ( $vps, $errors ) = $vv->generatePipelines ( $this->getViskoQuery () );
	...
	private function extractPipelines($viskoPipelines) {
		...
		foreach ( $viskoPipelines as $vp ) {
			$p = new Pipeline ( $this->getID (), $vp->getViewURI (), $vp->getViewerURI (), $vp->getToolkitThatGeneratesView (), $vp->getRequiresInputURL (), $vp->getOutputFormat (), $vp->getServices (), $vp->getViewerSets () );
	 ...
	 *        	the name of the parameter (URI) to add.
	 * @param String $value
	 *        	the new value of the parameter Binding
	 */
