...
/**
 * Store and retrieve Pipeline and PipelineStatus objects from the database.
 * 
 * @author awknaust
 *        
 */
class PipelineManager extends Manager {
	 ...
	 *
	 * @param
	 *        	Pipeline pipeline -- a pipeline without an ID
	...
	public function insertPipeline($pipeline) {
		...
		if (! ($stmt = $conn->prepare ( "INSERT INTO Pipelines (queryID, viewURI, 
			...
			$stmt->bind_param ( 'issssi', $qid, $pipeline->getViewURI (), $pipeline->getViewerURI (), $pipeline->getToolkitURI (), $pipeline->getOutputFormat (), $pipeline->getRequiresInputURL () );
	...
	private function insertServices($pipeline) {
		...
		if (! ($stmt = $conn->prepare ( "INSERT INTO PipelineServices (pipelineID, serviceID, 
					position)
				  SELECT ?, id, ? 
				  FROM Services 
				  WHERE URI = ?" ))) {
	...
	private function insertViewerSets($pipeline) {
		...
		if (! ($stmt = $conn->prepare ( "INSERT INTO PipelineViewerSets (pipelineID, ViewerSetID)
				  SELECT ?, id 
				  FROM ViewerSets 
				WHERE URI = ? " ))) {
			$this->handlePrepareError ( $conn );
	...
	private function collectViewerSets($pid) {
		 ...
		 */
		if (! ($stmt = $conn->prepare ( "SELECT ViewerSets.URI
	...
	private function collectServices($pid) {
		...
		/*
		 * Get all Service URIs for this pipeline and order them by their initial ordering
		 */
		if (! ($stmt = $conn->prepare ( "SELECT Services.URI
	...
	/**
	 * Get a pipelinestatus for this pipeline.
	 * It will be null
	...
	public function getPipelineStatusForPipeline($pipeline) {
		...
		if (! ($stmt = $conn->prepare ( "SELECT id, resultURL,
			...
			if (! $stmt->execute ()) {
				...
				$stmt->bind_result ( $id, $resultURL, $serviceIndex, $completedNormally, $dateExecuted );
				
				while ( $stmt->fetch () ) {
					;
				}
				
				$pipeStatus = new PipelineStatus ( $pipeline->getID (), $completedNormally, $resultURL, $serviceIndex, new DateTime ( $dateExecuted ), $id );
	...
	public function insertPipelineStatus($pipelineStatus) {
		...
		if (! ($stmt = $conn->prepare ( "INSERT INTO `PipelineStatuses` (pipelineID, resultURL,
					serviceIndex, completedNormally, dateExecuted )
				VALUES(?, ?, ?, ?, NOW())" ))) {
			return $this->handlePrepareError ( $conn );
		} else {
			$stmt->bind_param ( 'isii', $pipelineStatus->getPipelineID (), $pipelineStatus->getResultURL (), $pipelineStatus->getLastServiceIndex (), $pipelineStatus->completedNormally () );
