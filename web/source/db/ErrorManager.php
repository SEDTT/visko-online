...
class ErrorManager extends Manager{
	...
	public function __construct(){
		...
		/* An error handler must be registered for each error type. Also register
		 * for abstract types since they can be counted, if not stored.
		 */
	...
	*/
	 ...
	 * 
	 * @param String $clsName name of error class as String
 ...
 * the database. Because the Error classes are in a big hierarchy as are the error tables
	...
	public function store($error){
		...
		if(!($stmt = $conn->prepare("INSERT INTO `Errors` (userID, message, 
			...
			$stmt->bind_param('isi',
				$error->getUserID(),
				$error->getMessage(),
				$error->getTimeOccurred()->getTimestamp()
			);
