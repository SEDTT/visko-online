...
/**
 * These exceptions are created when the Backend cannot be reached (it isn't running, offline, etc.)
 *
 * @author awknaust
 *        
 */
 ...
 * the responses.
 * Deals primarily with Visko** objects.
 *
 * @author awknaust
 *        
 */
class ViskoVisualizer {
	 ...
	 *        	the query from which to generate pipelines
	 *        	
	 ...
	 *
	 * @param object $visResponse
	 *        	a json decoded VisualizatonResponse
	 */
	private function getErrors($visResponse) {
		return $visResponse->errors;
		/*
		 * $viskoErrors = []; foreach ($visResponse->errors as $verr){ $ve = new ViskoError(); $ve->fromJson($verr); $viskoErrors[] = $ve; } return $viskoErrors;
		 */
	}
	...
	/**
	 * Joins a base URL with an extension.
	 * I.e. 'http://localhost/' and 'query.php'
	 ...
	 *        	array of post keys->data parameters
	 *        	
...
class IdentifiedPipeline extends ViskoPipeline implements JsonCerializable {
	...
	public function __construct($id, $vpipeline) {
		...
		parent::init ( $vpipeline->viewURI, $vpipeline->viewerURI, null, null, null, $vpipeline->services, $vpipeline->viewerSets );
		$this->id = $id;
	}
