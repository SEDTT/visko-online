...
class Pipeline {
	 ...
	 *
	 * @param
	 *        	queryID int the ID of the query that generated this Pipeline
	 * @param
	 *        	viewURI String a View URI?
	 * @param
	 *        	viewerURI String a viewer URI?
	 * @param
	 *        	toolkit String the URI of the toolkit that generated the View
	 * @param
	 *        	requiresInputURL boolean whether this pipeline requires input
	 * @param
	 *        	outputFormat String URI the outputFormat of this pipeline
	 * @param
	 *        	services array(String) an ordered set of URIs that are this pipeline's steps
	 * @param
	 *        	viewerSets array(String) a set of viewer set URIs ?.
	 * @param
	...
	public function __construct($queryID, $viewURI = null, $viewerURI = null, $toolkit = null, $requiresInputURL = null, $outputFormat = null, $services = array(), $viewerSets = array(), $id = null) {
		...
		$vp->init ( $viewURI, $viewerURI, $toolkit, $requiresInputURL, $outputFormat, $services, $viewerSets );
	 ...
	 *
	 * @param Query $generatingQuery
	 *        	the query that generated this pipeline object.
	...
	public function execute($generatingQuery) {
		...
		try {
			...
			list ( $vps, $errors ) = $vv->executePipeline ( $this->getID (), $vquery, $this->getViskoPipeline () );
			...
			if ($errors != null && count ( $errors ) > 0) {
				...
				// by them
				throw new ServiceExecutionError ( $uid, $this->getID (), $vps->getServiceIndex (), $this->getServices ()[$vps->getServiceIndex ()] );
			} else {
				assert ( $vps != null );
				
				// create a pipelinestatus from the response
				$ps = new PipelineStatus ( $this->getID (), $vps->getCompletedNormally (), $vps->getResultURL (), $vps->getServiceIndex () );
	...
	private function inspectErrors($uid, $pipelineErrors) {
		...
		if (array_key_exists ( 'InputDataURLError', $etypes )) {
			...
			throw new InputDataURLError ( $uid, $this->getID (), $err->inputDataURL );
		} 

		else if (array_key_exists ( 'PipelineExecutionTimeoutError', $etypes )) {
			$err = $etypes ['PipelineExecutionTimeoutError'];
			throw new ServiceTimeoutError ( $uid, $this->getID (), $err->serviceIdx, $this->getServices ()[$err->serviceIdx] );
		} 		
