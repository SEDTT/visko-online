 ...
 * 
 * In json a query can be deserialized from either all of its fields, or from
 * the VSQL text. If using the VSQL text directly, simply pass "vsql : text",
 * and other fields will be ignored.
 * 
 * @author awknaust
 * 
 */
public class QueryAdapter implements JsonSerializer<Query>,
		JsonDeserializer<Query> {
	...
	public Query deserialize(JsonElement json, Type type,
		...
		if (vsql != null && vsql.isJsonPrimitive()) {
			...
			if ((jFormatURI == null || !jFormatURI.isJsonPrimitive())
					|| (jViewerSetURI == null || !jViewerSetURI
							.isJsonPrimitive())) {
				throw new JsonParseException(
						"Missing VSQL or viewerSetURI/formatURI");
			} else {
				// TODO is dataset really optional??
				if (jDatasetURL == null || !jDatasetURL.isJsonPrimitive()) {
					query = new Query(null, jFormatURI.getAsString(),
							jViewerSetURI.getAsString());
				} else {
					query = new Query(jDatasetURL.getAsString(),
							jFormatURI.getAsString(),
							jViewerSetURI.getAsString());
				}
			}
			...
			if (jParameterBindings != null) {
				...
				HashMap<String, String> parameterBindings = context
						.deserialize(jobj.get("parameterBindings"),
								typeOfHashMap);
